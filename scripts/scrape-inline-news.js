...
async function scrapeOneBlading() {
  try {
    const html = await (await fetch("https://oneblademag.com/", UA)).text();
    const $ = cheerio.load(html);
    const items = [];
    ...
    $("article, .post").each((i, el) => {
      ...
      if (title && link) {
        items.push({
          title,
          url: link.startsWith("http") ? link : `https://oneblademag.com${link}`,
          source: "OneBlading",
          snippet,
          date
        });
...
async function scrapeReddit() {
  try {
    const json = await (await fetch("https://www.reddit.com/r/rollerblading/new.json?limit=30", UA)).json();
...
async function scrapeYouTube() {
  try {
    const xml = await (await fetch("https://www.youtube.com/feeds/videos.xml?search_query=inline+skating", UA)).text();
    const items = [];
