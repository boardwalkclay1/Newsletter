...
async function scrapePlanetRollerSkate() {
  const url = "https://planetrollerskate.com/blog/";
...
async function scrapeMoxi() {
  const url = "https://moxiskates.com/blogs/news";
  const html = await (await fetch(url)).text();
  const $ = cheerio.load(html);
  const items = [];
...
async function scrapeReddit() {
  const url = "https://www.reddit.com/r/Rollerskating/new.json?limit=30";
...
async function scrapeYouTube() {
  const url = "https://www.youtube.com/feeds/videos.xml?search_query=roller+skating";
